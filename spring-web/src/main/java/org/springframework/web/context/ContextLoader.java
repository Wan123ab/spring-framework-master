...
public class ContextLoader {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					if (cwac.getParent() == null) {
						...
						/**
						 * 获取父容器并进行设置，loadParentContext(ServletContext servletContext)是1个模板方法，
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		/**
		 * 返回一个WebApplicationContext实现类的Class，默认是XMLWebApplicationContext，
		 * 如果在web.xml中指定了自定义的contextClass，那么返回自定义实现类的Class，注意，自定义的实现类
		...
		/**
		 * 调用构造函数生成1个WebApplicationContext实例并返回
		 * 有2中情况：
		 * 1、普通的Java类，使用无参构造实例化
		 * 2、Kotlin类，使用主要构造函数（可能有一些默认参数）实例化
		 */
